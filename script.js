...
function calcPotions(actualLevel, desiredLevel, tier) {
  ...
  const xpMedium = Math.round(
    (levels[desiredLevel] - levels[actualLevel]) / potions[tier].medium
  );
  const modXpMedium = Math.round(
    (levels[desiredLevel] - levels[actualLevel]) % potions[tier].medium
  );
  const restOfMedium = Math.ceil(modXpMedium / potions[tier].small);

  const xpLarge = Math.floor(
    (levels[desiredLevel] - levels[actualLevel]) / potions[tier].large
  );
  const modXpLarge = Math.round(
    (levels[desiredLevel] - levels[actualLevel]) % potions[tier].large
  );
